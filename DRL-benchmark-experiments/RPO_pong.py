...
seeds = [0, 1, 2, 25,3]
for seee_ in tqdm(seeds):
    rnn = []
    @dataclass
    class Args:
        exp_name: str = 'RPO_pong'
    ...
    class Agent(nn.Module):
        ...
        def get_action_and_value(self, x, action=None):
            ...
            if action is None:
                ...
                z = torch.FloatTensor(action.shape).uniform_(-self.rpo_alpha, self.rpo_alpha).to(device)
                action = action + z
                action = torch.clamp(action, 0, 5).long()
                probs = Categorical(logits=logits)
    ...
    envs = ss.concat_vec_envs_v1(
        env, 2 // 2, num_cpus=0, base_class="gymnasium"
    )
    ...
    for update in range(1, num_updates + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                rnn.append(b_returns[mb_inds].item())
    ...
    df.to_csv(f'TRPO_pong_{seee_}.csv')
    print('done seed: ', seee_)
    envs.close()
    writer.close()