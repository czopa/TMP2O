...
def parse_args():
    ...
    parser.add_argument("--total-timesteps", type=int, default=2000000,  # CleanRL default: 2000000
...
if __name__ == "__main__":
    ...
    for update in tqdm(range(1, num_updates + 1)):
        ...
        # # hide any unused axes (here 6 slots, only 5 series)
        ...
        writer.add_scalar("losses/rewards", b_returns.mean().item(), global_step)
...
loss_df = pd.DataFrame({
    'overall_loss':agentlll,
    'rewards':[x.item() for x in rnn],
    'policy_loss':pgllll,
    'entropy_loss': elll
})
loss_df.to_csv('PPO_Pong_50.csv')

def record_video(env_id, agent, run_name, num_frames=5000):
    ...
    for _ in range(num_frames):
        ...
        with torch.no_grad():
            ...
            noise = np.random.normal(loc=0.0, scale=noise_sigma, size=obs_tensor.shape)
            obs_tensor = obs_tensor + torch.tensor(noise).float().to(device)
        ...
        if all(terminations.values()) or all(truncations.values()):
            break
    video_path = f"{run_name}.mp4"
    imageio.mimwrite(video_path, frames, fps=30)
    print(f"Saved video to {video_path}")
    env.close()

import imageio
record_video('pong_v3', agent, run_name)
